...
extension ViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        headView.headViewModel = viewModel.headViewModelObservable.value
        return headView
    }
...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.cellViewModelsObservable.value.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = CustomCell.getCustomCell(tableView: tableView)
        cell.cellViewModel = viewModel.cellViewModelsObservable.value[indexPath.row]
        return cell
    }
...
extension ViewController: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if indexPath.row >= viewModel.cellViewModelsObservable.value.count-2 {
            loadMore()
        }
    }
